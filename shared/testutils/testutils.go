...
func BackupAndRestoreWithId(t testing.TB, id string) func() {
	...
	renameFiles := []string{
		...
		path.Join(homedir, data.GetHishtoryPath(), data.STATE_PATH),
		path.Join(homedir, data.GetHishtoryPath(), data.LEGACY_CONFIG_PATH),
		path.Join(homedir, data.GetHishtoryPath(), data.LEGACY_CONFIG_PATH+".old"),
